...
if ($Help) {
    ...
    Write-Host "  -Port <port>     Upload firmware to specified port (e.g., COM3)" -ForegroundColor White
    ...
    Write-Host "  .\build_esp32.ps1 -Port COM3        # Compile and upload firmware" -ForegroundColor Gray
...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($Port) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  - Web interface: http://[device-ip]/update" -ForegroundColor White
            Write-Host "  - Arduino IDE: Select network port after first boot" -ForegroundColor White
            Write-Host "  - Hostname: breadmaker-controller.local" -ForegroundColor White
        } else {
        ...
        Write-Host "  .\build_esp32.ps1 -Port COM3        # Upload firmware only" -ForegroundColor Gray
        Write-Host "  .\build_esp32.ps1 -Port COM3 -UploadData # Upload firmware and data" -ForegroundColor Gray
